...
that created the Azure Pipeline.)  They provide only a certain number of
CPUs, and they additionally throttle work based on CPU usage per day.
...
Azure's mechanism for sharing files/directories between stages or jobs within a pipeline is to use "artifacts".  They are somewhat limited: 
"Subdirectories of the specified path will not be published. Wildcards in the path are not supported."
