...
GitHub Actions offers free 2000 Linux minutes per month for public repositories.
  At 1 hour per build, that's one build per day.  (In other words, it's very little!)
...
A problem with GitHub Actions is that it offers only a "Re-run all jobs" option,
