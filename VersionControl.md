#summary Wisdom about version control systems such as CVS, Git, Mercurial (Hg), RCS, and Subversion (SVN).
...
(the first ignores whitespace changes):
```
...
```
  ...
  git reset --soft HEAD^
```
I'm not sure exactly what is the distiction between these.
To revise a commit before pushing it -- similarly to what "hg rollback" enbales:
 * do more edits
 * `git commit --amend`
...
```
  ...
  git bundle create ../yourRepo.bundle master     // for unpushed changes
```
