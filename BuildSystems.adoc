...
```
(For the Checker Framework, the command is `./gradlew deployArtifactsToLocalRepo`.)
...
Framework runs (but this has an effect only if Gradle provides provides no
`-Xmx` argument when greating the forked process):
```
JAVA_TOOL_OPTS="-Xmx2g" ./gradlew build javadoc
```
To provide more memory (maximum heap size) to Gradle itself from the command
