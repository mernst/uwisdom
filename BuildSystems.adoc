...
To upgrade to Gradle 5:
 * First upgrade to gradle 4.10:  gradle wrapper --gradle-version 4.10
 * Ensure that build passes, look for warnings: ./gradlew build
 * Look for deprecations:
    * Add to `build.gradle`:
      buildScan { termsOfServiceUrl = 'https://gradle.com/terms-of-service'; termsOfServiceAgree = 'yes' }
     * Run `./gradlew help --scan` and browse to the URL.  If the Deprecations tab appears, see it.  If the Deprecations tab does not apper, you are set.
     * Remove `buildScan` line from `build.gradle`
 * Upgrade to gradle 5:  ./gradlew wrapper --gradle-version 5.3.1
 * Ensure that build passes, look for warnings: ./gradlew build
...
To print/show the classpath tha Gradle uses:
```
...
task printTestClasspath {
  ...
  doLast {
    ...
    sourseSets.main.runtimeClasspath.each { println it }
  }
}
```
...
```
or for version 2.8.0:
VER=2.8.0 && \
(cd $CHECKERFRAMEWORK && ./gradlew copyJarsToDist) && \
\cp -pf $CHECKERFRAMEWORK/checker/dist/checker.jar ~/.gradle/caches/modules-2/files-2.1/org.checkerframework/checker/${VER}/d40dfb699cc125d55daf928733036459bbc3948f/checker-${VER}.jar && \
\cp -pf $CHECKERFRAMEWORK/checker/dist/checker-qual.jar ~/.gradle/caches/modules-2/files-2.1/org.checkerframework/checker-qual/${VER}/35b7432f3cc3ff2addabfa537867cb6b190d493b/checker-qual-${VER}.jar && \
\cp -pf $CHECKERFRAMEWORK/checker/dist/jdk8.jar ~/.gradle/caches/modules-2/files-2.1/org.checkerframework/jdk8/${VER}/e52ab54bf7fef2257a7b3d01c2c8e64d1d295898/jdk8-${VER}.jar
.
