...
A problem with the Gradle signing plugin is that it requires a legal GPG keyring.
(I don't know whether the Maven signing plugin also requires this.)
If the file doesn't exist or is not valid, the gradle build terminates.
But signing is only needed when uploading artifacts to Maven Central, and some users may not have a keyring.
.
Here are some possibilities, from best to worst:
 * split the gradle file into parts, and include them in one another.  This is a bit gross since only a gradle script can be included, but only a gradle buildfile can contain a `plugins { ... }` block, and referring to a plugin outside the `plugins { ... }` block is a bit gross.
 * commit a dummy keyring to the repository, refer to it in the local gradle.properties, and the user's ~/gradle.properties can override it for any user who wants to upload to Maven Central.  A problem is that using a local pathname yields a gradle warning, which we don't want.
 * use conditional signing.  This does not help -- perhaps the artifacts won't be signed on any particular run, but the signing plugin still requires the file to exist.

Gradle's "maven-publish" plugin does no signing, so it cannot be used with
Maven Central.
