...
To upgrade Gradle:
 * gradle wrapper --gradle-version 4.10.3 && ./gradlew build
 * Look for warnings
 * Look for deprecations (see below)
 * ./gradlew wrapper --gradle-version 5.6.4 && ./gradlew build
 * Look for warnings
 * Look for deprecations (see below)
 * ./gradlew wrapper --gradle-version 6.3 && ./gradlew build
 * Look for warnings
 * Look for deprecations (see below)
.
...
```
For Gradle 2-5: Add to `settings.gradle`:
```
