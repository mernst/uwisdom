...
To copy the Checker Framework to the Maven local repository:
```
VER=2.5.4 && \
(cd $cf && ./gradlew copyJarsToDist) && \
\cp -pf $cf/checker/dist/checker.jar ~/.m2/repository/org/checkerframework/checker/${VER}/checker-${VER}.jar && \
\cp -pf $cf/checker/dist/checker-qual.jar ~/.m2/repository/org/checkerframework/checker-qual/${VER}/checker-qual-${VER}.jar && \
\cp -pf $cf/checker/dist/jdk8.jar ~/.m2/repository/org/checkerframework/jdk8/${VER}/jdk8-${VER}.jar
```
To clean the Maven local repository (rarely needed):
```
VER=2.5.4 && \
rm -rf \
...
```
To test a new version of a Gradle dependecy locally without creating a
local repository, just overwrite the `.jar` file in a subdirectory of
~/.gradle/caches/modules-2/files-2.1/

In Travis, it's a good idea to enable Gradle caching.  It reduces bandwidth
and time usage, and more importantly it reduces transient failures that
happen due to loss/reduction of network connectivity.
```
...
To enable Gradle caching of downloaded artifacts on Travis CI:
```
before_cache:
  - rm -f  $HOME/.gradle/caches/modules-2/modules-2.lock
  - rm -fr $HOME/.gradle/caches/*/plugin-resolution/
...
```
docker run -v $HOME/.gradle:/root/.gradle USER/IMAGE-NAME /bin/bash -c "..."
```
