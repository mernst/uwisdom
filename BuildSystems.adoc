...
(Find projects to upgrade via `locate gradlew`
then `(progn (delete-matching-lines "-fork-[^m]" nil nil t)
 ...
 * Look for warnings and deprecations
 * ./gradlew wrapper --gradle-version 8.0.1 && ./gradlew build --warning-mode=all
 * Look for warnings and deprecations
.
To determine details for deprecations:
