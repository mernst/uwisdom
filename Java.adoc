...
```
The purpose of `{@code ...}` is to prevent HTML interpretation; <, >, &amp;,
are passed through unchanged.
To quote/escape the @ (at-sign) symbol in Javadoc, use {@literal @} or &#064;
This is needed only at the beginning of a line to prevent it from being
interpreted as starting a Javadoc tag.  It is needed even within
<pre>{@code ...}</pre>.  Another approach is to put &nbsp; before the literal @.
...
In a Javadoc @param clause (and related clauses), the initial text is a
sentence fragment that starts with a lowercase (not capital) letter and
does not end with a period unless followed by another sentence.
...
----
  J2SE 9.0 = 53 (0x35 hex)    [as of 10/2016, General Availability (GA) July 2017]
