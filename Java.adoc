...
To limit/increase the number of errors that javac will print (default 100) use, -Xmaxerrs N
...
Another approach is to put &nbsp; before the literal @.
This is needed only at the beginning of a line to prevent it from being
interpreted as starting a Javadoc tag.  Neither {@literal @} nor &#064; works
within `<pre>{@code ...}</pre>`; for code blocks, use
`<pre>{@code ... \n{@literal @}...}</pre>` though note that the space after
`@literal` will make the result one space more indented than you expected.
