...
highly error-prone.  Therefore, almost all shell scripts should start with
```
set -e 
```
...
To resolve Ubuntu popup "please update the application snap-store":
```
snap-store --quit
sudo snap refresh
```
...
To address an Ubuntu warning "Pending update of snap store":
```
