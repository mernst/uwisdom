...
```
To print a USGS quad map on 8 sheets of letter paper:
```
  pdfcrop --margins '-200 -50 -150 -150' foo.pdf
  pdftops foo-crop.pdf
  poster -v -mletter -s1.2 foo-crop.ps > foo-scaled1.2.ps
  ps2pdf foo-scaled1.2.ps
```
...
It's better to use AsciiDoc format and the Asciidoctor processor, when possible.
