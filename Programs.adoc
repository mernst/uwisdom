...
```
  openssl enc -aes128 -e -in file -out file.aes128
  openssl enc -aes128 -d -in file.aes128 -out file.decrypted
```
Optional argument:  -k secretkey
For other ciphers, change -aes128 to -bf or -des3
Concrete example:
```
  openssl enc -aes128 -e -in wisdom.machines.decrypted -out wisdom.machines.aes128 && chmod og-rwx wisdom.machines.decrypted && rm -f wisdom.machines.decrypted
  openssl enc -aes128 -d -in wisdom.machines.aes128 -out wisdom.machines.decrypted && chmod og-rwx wisdom.machines.decrypted 
```

To encrypt a file symmetrically with GPG:
(I don't like this because it gets entangled with my 
...
pwgen -N1: generate one random password (Ubuntu)
 ...
 * from the command line, use `realpath`
...
For converting (GitHub-style) markdown format to HTML, an alternative to
`markdown` is `grip` (after which browse to localhost:5000 , but that hung
my Emacs when I tried it) or `grip --export` which exports to
`<path>.html`.
Markdown is idiosyncratic, so I think it's better to use AsciiDoc format
and the Asciidoctor processor, when possible.
...
To retrieve a file from a remote URL, the following two commands are the
same except that `wget` preserves the remote timestamp and `curl` does not.
```
  wget URL
  curl -s -O URL
```
