...
To run an interactive bash shell in a docker image:
```
  docker images
  docker run -it <image> /bin/bash
```
or try just (but this didn't work on MacOS):
```
  docker run OWNER/NAME /bin/bash
```
...
```docker rmi ID```
To remove/delete all docker images:
```
docker stop $(docker ps -a -q)
docker rm $(docker ps -a -q)
