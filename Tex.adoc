...
To change the font and line spacing for figure captions, do:
```
\usepackage{setspace}
\usepackage{caption}
\captionsetup{labelfont={small,bf}, textfont={small,bf,stretch=0.90}, labelsep=colon, margin=0pt}
```
...
setting \itemsep and \parskip), see
link:TexListSpacing.adoc[TexListSpacing.adoc].
The most common incantation is
...
Using `times` instead of `pslatex` is fine, though.
...
The lstlisting package by default puts its line numbers in the column gutter rather than where it belongs.  To fix this, use:
```
