...
Run `git remote prune origin` periodically to remove branches that have been
...
To pull pull recent changes to master into a branch:
```
...
In a GitHub pull request, if the upstream has changed, then just pulling the upstream changes into the pull request shows all those pulled changes in the code diffs.  It is a bug in GitHub pull requests that they don't recognize commits pulled from upstream.
.
If you want to see the changes against the most recent version of upstream, you need to:
