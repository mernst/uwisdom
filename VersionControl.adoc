...
To search the git history:
```
  git log -G"WHERE_DID.*"
```
returns all commits that match the given regular expression.
```
  git log -S"WHERE_DID_IT_GO"
```
returns all commits with a different number of occurrences of the search
