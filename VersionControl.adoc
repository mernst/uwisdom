 ...
 * merge driver: A merge driver is called whenever no two of
   ...
   given patterns.
 * merge tools (mergetools):  A merge tool never runs automatically.  If a user
   issues the command `git mergetool`, then the mergetool is run on all
   conflicted files (one at a time) and observes the conflict markers.  It gets
   the names of the original files.  A git mergetool is never invoked on a file
   ...
   mergetool will never reduce the number of clean-but-incorrect merges.  It
   will only change some conflicted merges into correct or incorrect merges.
